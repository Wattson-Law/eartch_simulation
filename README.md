...
- Sprite Sheet / 序列帧 + `requestAnimationFrame`：动物、天气和捕食由连续行为状态机驱动；CSS 负责界面过渡
- SVG：种群数量曲线（无第三方图表库）
- 纯规则中文 NLP（无后端、无 LLM API、无 Three.js）
- Oxlint 静态检查，`npm run build` 执行 TypeScript 检查与 Vite 构建

## 视觉边界与实现状态（2026-09-21）
...
- **已完成渲染**：地球入口用绘本世界地图做连续 Canvas 2D 球面投影，外围由深空底色、青紫星云、柔和银河带和远中近三层星群构成，并叠加大气辉光与轨道线。黄石场景以拉马谷的雪山、鼠尾草草甸、针叶林、蜿蜒河流和前景岸线分层合成；山体和森林图层保留深度拼接，河流使用跨全景连续曲线，避免镜像接缝反向。水面鱼群、植物轻摆、云漂移、动物动作和季节/天气状态连续变化。缺失素材仍有程序绘制回退；原地球图集保留在素材验收页。
...
- **手机与桌面观察**：场景是约 2.4 个视口宽的横向全景，可用鼠标、触控、滚轮、方向键、Home / End 左右浏览；动物按视口宽度缩放并显示代表个体。捕食记录位于场景下方，不遮挡风景或统计。工具栏支持暂停 / 继续观察，停止时间推进与动物、环境动画。
...
- **自然活动与捕食**：代表个体拥有连续位置、加减速、目的地和独立行为节奏；狼会远距离观察、低身接近、短暂追逐并重新聚拢，草食动物觅食、短跳、饮水、警觉与逃离。步态跟随移动距离，路径避开河流与岩石，动物和植物按纵深遮挡。宏观捕食压力连续计算，可见事件按观察窗口采样，避免重复追逐；多数遭遇以脱险结束，成功捕食读取已发生的种群事件，并在植被遮挡或画外收束，场景下方同步显示正在发生的行为。
- **动作与遮挡细化**：9 组移动和特殊动作由 8 张原始姿态补为 32 帧，保留周期和落脚锚点；相邻帧补间、减速、弧线转向、朝向交叉淡化、呼吸和起伏共同避免僵硬翻面。动物沿可达路径进入草丛，穿过叶片透明轮廓逐步显露，捕食后不会在空地突然淡入。
...
src/
  ...
  nlp/     中文规则解析与巡护员话术
  ui/      地球、生态区、巡护故事线、曲线、简报、聊天、级联面板、捕食动画
  App.tsx  单页装配
