...
- **自然活动与捕食**：`visualSlice.ts` 为每个可见物种只保留一个代表个体，沿预先绘制的安全观察线缓慢移动；日常动作只在原地呼吸、觅食、饮水、坐卧、藏匿和探出，只有事件阶段短暂切换为潜行/追逐姿态。画面不运行逐只寻路、避障、碰撞或捕食物理，狼和猎物不会在河面上追撞。宏观捕食压力仍由确定性模型计算；视觉事件只读取已发生的 `lastPredation`，按潜行 → 追逐 → 倒地/进食 → 脱险切片表现，动画结束不再次扣减种群。
